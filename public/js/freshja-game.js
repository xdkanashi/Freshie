...
const touchTrailColor = "rgba(255, 255, 255, 0.9)"; // Более контрастный цвет
const touchTrailThickness = 10; // Увеличим толщину
const touchPointLife = 120;
const touchPoints = [];
const targetRadius = 40;
const targetHitRadius = 60;
const makeTargetGlueColor = (target) => "rgb(170,221,255)";
const fragRadius = targetRadius / 4;
...
// Local Storage
const highScoreKey = "__freshja__highScore";
const getHighScore = () => {
...
function renderSlowmoStatus(percentRemaining) {
    ...
    console.log("Slowmo remaining:", percentRemaining); // Отладка
}
...
// Add score saving to leaderboard
...
function tick(width, height, simTime, simSpeed, lag) {
    ...
    if (pointerIsDown && lastPointer && !lastPointer.touchBreak) {
        ...
        pointerDeltaScaled.x = pointerDelta.x * (1 / (simSpeed * 0.75 + 0.25));
        pointerDeltaScaled.y = pointerDelta.y * (1 / (simSpeed * 0.75 + 0.25));
    }
    const pointerSpeed = Math.hypot(pointerDelta.x, pointerDelta.y);
    const pointerSpeedScaled = pointerSpeed * (1 / (simSpeed * 0.75 + 0.25));
    console.log("Pointer Speed Scaled:", pointerSpeedScaled);
    ...
    if (pointerIsDown) {
        ...
        console.log("Touch Points:", touchPoints); // Отладка
    }
        ...
        for (let ii = 1; ii <= hitTestCount; ii++) {
            ...
            if (distance <= targetHitRadius) {
                if (!target.hit) {
                    ...
                    if (pointerSpeedScaled > minPointerSpeed / 2) {
                        target.health--;
                        incrementScore(10);
                        ...
                        if (target.health <= 0) {
                            ...
                            createBurst(
                                target,
                                1 + (target.maxHealth - 1) * 0.5
                            );
                            sparkBurst(hitX, hitY, 12, sparkSpeed);
                            if (target.wireframe) {
...
function draw(ctx, width, height, viewScale) {
    ...
    // Улучшаем отображение touchTrail
    ctx.strokeStyle = "rgba(255, 255, 255, 0.9)"; // Более контрастный цвет
    const touchPointCount = touchPoints.length;
    console.log("Touch Points Count for Drawing:", touchPointCount); // Отладка
    for (let i = 1; i < touchPointCount; i++) {
        ...
        if (current.touchBreak || prev.touchBreak) {
            console.log("Touch break detected, skipping line segment");
            continue;
        }
        const scale = current.life / touchPointLife;
        ctx.lineWidth = scale * 10; // Увеличиваем толщину линии
        ctx.beginPath();
        ctx.moveTo(prev.x, prev.y);
        ctx.lineTo(current.x, current.y);
        ctx.stroke();
        console.log(
            `Drawing line from (${prev.x}, ${prev.y}) to (${current.x}, ${current.y})`
        ); // Отладка
    }
...
function setupCanvases() {
    ...
    handleResize();
    window.addEventListener("resize", handleResize); // Убедимся, что работает при изменении размера

    canvas.style.position = "absolute"; // Устанавливаем абсолютное позиционирование
    canvas.style.top = "0";
    canvas.style.left = "0";
    canvas.style.zIndex = "1"; // Убедимся, что канвас под HUD и меню
...
function handleCanvasPointerDown(x, y) {
    ...
    if (!pointerIsDown) {
        ...
        pointerScreen.x = x;
        pointerScreen.y = y;
        console.log("Pointer Screen set to", pointerScreen);
